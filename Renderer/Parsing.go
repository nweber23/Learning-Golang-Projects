...
func LoadSceneFile(filename string) (*Scene, error) {
	...
	defaultTexture := Rendering.NewColorTexture(defaultColor)
	...
	for _, meshData := range sceneData.Meshes {
		...
		if meshData.Texture != "" {
			texture, err := Rendering.LoadTextureFile(path.Join(rootDir, meshData.Texture))
			if err != nil {
	...
	for _, objData := range sceneData.Objects {
		...
		obj := Rendering.NewObject(mesh)
		obj.Scale = Math.Vec3FromArray(objData.Scale)
		obj.Rotation = Math.Vec3FromArray(objData.Rotation).ToRadians()
		obj.Translation = Math.Vec3FromArray(objData.Position)
