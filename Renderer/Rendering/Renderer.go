...
func calculateTileBoundaries(tile uint, numTiles uint, width, height int) (start, end Math.Vec2) {
	if numTiles == 1 {
		return Math.Vec2{0, 0}, Math.Vec2{float32(width), float32(height)}
	}
...
func NewRenderer(fb *FrameBuffer, parallel bool) *Renderer {
	...
	for i := uint(0); i < r.numTiles; i++ {
		...
		r.tileBounds[i] = [2]Math.Vec2{start, end}
	}
...
func (r *Renderer) identifyTriangleTiles(points *[3]Math.Vec4, tileNums *[maxTiles]uint8) (n int) {
	var (
...
func (r *Renderer) projectObject(object *Object, camera *Camera) {
	worldMatrix := Math.NewWorldMatrix(object.Scale, object.Rotation, object.Translation)
	viewMatrix := Math.NewViewMatrix(camera.Position, camera.Direction, camera.Up)
	perspectiveMatrix := Math.NewPerspectiveMatrix(r.fovY, r.aspectX, r.zNear, r.zFar)

	mvpMatrix := Math.NewIdentityMatrix()
	...
	screenMatrix := Math.NewScreenMatrix(r.fb.Width, r.fb.Height)
	lightDirection := Math.Vec3{X: -1, Y: 1, Z: 1}.Normalize()

	// Transform the bounding box to clip space
	bbox := object.BoundingBox
	Math.MatrixMultiplyVec4Batch(&mvpMatrix, bbox[:])
	...
	var (
		...
		vertices        [3]Math.Vec4
		vertexIntensity [3]float32

		// New points after frustum clipping
		clipVertices  [maxClipPoints][3]Math.Vec4
	...
	Math.MatrixMultiplyVec4Batch(&mvpMatrix, object.TransformedVertices)
	...
	Math.MatrixMultiplyVec4Batch(&worldMatrix, object.WorldFaceNormals)
	Math.MatrixMultiplyVec4Batch(&worldMatrix, object.WorldVertexNormals)
	...
	for fi := range object.Faces {
		...
		if r.Lighting {
			if hasVertexNormals && !r.FlatShading {
				...
				vertexIntensity[0] = ambientStrength + vn0.Dot(lightDirection)*diffuseStrength
				vertexIntensity[1] = ambientStrength + vn1.Dot(lightDirection)*diffuseStrength
				vertexIntensity[2] = ambientStrength + vn2.Dot(lightDirection)*diffuseStrength
			} else {
				fn := object.WorldFaceNormals[fi].Normalize().ToVec3()
				diffuse := fn.Dot(lightDirection) * diffuseStrength
		...
		for i := 0; i < clipCount; i++ {
			...
			for j := range screenPoints {
				...
				Math.MatrixMultiplyVec4Inplace(&screenMatrix, &screenPoints[j])
