...
type Polygon struct {
	...
	Points    [maxClipPoints]Math.Vec4
	UVs       [maxClipPoints]UV
	Count     int
}

func (p *Polygon) AddVertex(v Math.Vec4, uv UV, intensity float32) {
...
func (p *Polygon) Triangulate(
	points *[maxClipPoints][3]Math.Vec4,
	...
	for i := 0; i < p.Count-2; i++ {
		...
		points[numOut] = [3]Math.Vec4{p.Points[0], p.Points[i+1], p.Points[i+2]}
...
func (p *Plane) DistanceToVertex(v Math.Vec4) float32 {
	return p.Normal.Dot(v) - p.Normal.Dot(p.Point)
}

// IsVertexInside tells if a point is inside or outside the plane.
func (p *Plane) IsVertexInside(q Math.Vec4) bool {
	return q.Sub(p.Point).Dot(p.Normal) <= 0
}

// Intersect returns a point between q0 and q1 intersect with the plane.
func (p *Plane) Intersect(q0, q1 Math.Vec4) (Math.Vec4, float32) {
	u := q1.Sub(q0)
	w := q0.Sub(p.Point)
	d := p.Normal.Dot(u)
	n := -p.Normal.Dot(w)
...
func NewFrustum(zNear, zFar float32) *Frustum {
	...
	return &Frustum{
		Planes: [6]Plane{
			PlaneLeft: {
				Point:  Math.Vec4{-1, 0, 0, 1},
				Normal: Math.Vec4{1, 0, 0, 1},
			},
			PlaneRight: {
				Point:  Math.Vec4{1, 0, 0, 1},
				Normal: Math.Vec4{-1, 0, 0, 1},
			...
			PlaneTop: {
				Point:  Math.Vec4{0, -1, 0, 1},
				Normal: Math.Vec4{0, 1, 0, 1},
			},
			PlaneBottom: {
				Point:  Math.Vec4{0, 1, 0, 1},
				Normal: Math.Vec4{0, -1, 0, 1},
			...
			PlaneNear: {
				Point:  Math.Vec4{0, 0, zNear, 1},
				Normal: Math.Vec4{0, 0, -1, 0},
			},
			PlaneFar: {
				Point:  Math.Vec4{0, 0, zFar, 1},
				Normal: Math.Vec4{0, 0, 1, 0},
			},
		},
...
func (f *Frustum) BoxVisibility(bbox *[8]Math.Vec4) int {
...
func (f *Frustum) ClipTriangle(
	pointsIn *[3]Math.Vec4,
	uvsIn *[3]UV,
	intensityIn *[3]float32,

	pointsOut *[maxClipPoints][3]Math.Vec4,
