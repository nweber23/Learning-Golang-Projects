...
func main() {
	...
	fb := Rendering.NewFrameBuffer(viewWidth, viewHeight)
	renderer := Rendering.NewRenderer(fb, parallel)
	renderer.DemoMode = demoMode
	filename := flag.Arg(0)
	...
	case ".obj":
		...
		scene = &Scene{}
		for i := range meshes {
			object := Rendering.NewObject(meshes[i])
	...
	for !rl.WindowShouldClose() {
		...
		if !demoMode {
			...
			if deltaX != 0 || deltaY != 0 {
				...
				yawQuaternion := Math.NewQuaternionFromAxisAngle(camera.Up, yaw)
				pitchQuaternion := Math.NewQuaternionFromAxisAngle(right, pitch)
